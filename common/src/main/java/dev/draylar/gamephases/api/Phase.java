package dev.draylar.gamephases.api;
...
import dev.draylar.illusion.api.IllusionReplacement;
import dev.draylar.illusion.api.IllusionTarget;
...
public class Phase {
    ...
    public Phase block(String blockPattern, Block replacement) {
        for (Block block : match(blockPattern, Registry.BLOCK)) {
            ...
            Illusion illusion = Illusion.create()
                    .map(new IllusionTarget(block), new IllusionReplacement(replacement.getDefaultState()))
                    .when(player -> !GamePhases.getPhaseData(player).phases$has(this.id))
                    .modifyDrops(true)
    ...
    public static Phase fromTag(NbtCompound tag) {
        ...
        NbtList items = tag.getList("Items", NbtElement.STRING_TYPE);
        NbtList blocks = tag.getList("Blocks", NbtElement.STRING_TYPE);
        NbtList dimensions = tag.getList("Dimensions", NbtElement.STRING_TYPE);
        NbtList entities = tag.getList("Entities", NbtElement.COMPOUND_TYPE);
        NbtList itemTags = tag.getList("ItemTags", NbtElement.STRING_TYPE);
        NbtList blockTags = tag.getList("BlockTags", NbtElement.STRING_TYPE);
